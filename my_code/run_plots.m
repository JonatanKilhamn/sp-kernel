...
loglog(sizesToPlot, stdQueryRuntimes, '-x', ...
    ...
    sizesToPlot, vorQueryRuntimes(end, :, 1), '-x', ...
    sizesToPlot, vorQueryRuntimes(end, :, 2), '-x')
...
loglog(sizesToPlot, stdTotalRuntimes, '-x', ...
    sizesToPlot, smpLstTotalRuntimes(3, :), '-x', ...
    sizesToPlot, smpLstTotalRuntimes(end, :), '-x', ...
    sizesToPlot, smpFstTotalRuntimes(3, :), '-x', ...
    sizesToPlot, smpFstTotalRuntimes(end, :), '-x', ...
    sizesToPlot, vorTotalRuntimes(end, :, 1), '-x', ...
    sizesToPlot, vorTotalRuntimes(end, :, 2), '-x')
legend('Standard', ...
    ['SampleLast (m=', num2str(ms(3)), ')'], ...
    ['SampleLast (m=', num2str(ms(end)), ')'], ...
    ['SampleFirst (m=', num2str(ms(3)), ')'], ...
    ['SampleFirst (m=', num2str(ms(end)), ')'], ...
    ['Voronoi (m=', num2str(ms(end)), ', p=', ...
    num2str(densities(1)), ')'], ...
    ['Voronoi (m=', num2str(ms(end)), ', p=', ...
...
legend('Standard, query', 'Standard, total', ...
    ...
    'SampleFirst, total', ...
    ['Voronoi, total (p=', num2str(densities(1)) ,')'], ...
    ['Voronoi, total (p=', num2str(densities(end)) ,')'], ...    
    'Location', 'NorthEastOutside')
...
legend('Standard', ...
    ...
    ['Voronoi, p=', num2str(densities(end))], ...
    'Location', 'NorthEastOutside')
title(['Shortest path distribution errors on ', dataset, num2str(graphSize)])
xlabel('Sample size')
...
legend('Standard', ...
    ...
    ['SampleFirst, m=', num2str(ms(end))], ...
    ['Voronoi, m=', num2str(ms(end)), ', p=', num2str(densities(1))], ...
    ['Voronoi, m=', num2str(ms(end)), ', p=', num2str(densities(2))], ...
    'Location', 'NorthEastOutside')
...
figure(9)
semilogy(sizesToPlot, smpFstTotalOps(1, :), '-x', ...
         sizesToPlot, smpFstTotalOps(end, :), '-x', ...
         sizesToPlot, vorTotalOps(end, :, 1), '-x', ...
         sizesToPlot, vorTotalOps(end, :, 2), '-x', ...
         sizesToPlot, vorQueryOps(end, :, 1), '--x', ...
         sizesToPlot, vorQueryOps(end, :, 2), '--x')
legend(['SampleFirst, total (m=', num2str(ms(1)), ')'], ...
       ['SampleFirst, total (m=', num2str(ms(end)), ')'], ...
       ['Voronoi, total (m=', num2str(ms(end)), '), p=', ...
       num2str(densities(1)), ')'], ...
       ['Voronoi, total (m=', num2str(ms(end)), '), p=', ...
       num2str(densities(2)), ')'], ...
       ['Voronoi, query (m=', num2str(ms(end)), '), p=', ...
       num2str(densities(1)), ')'], ...
       ['Voronoi, query (m=', num2str(ms(end)), '), p=', ...
       num2str(densities(2)), ')'], ...
       'Location', 'SouthEast')
    %'Standard', 'Standard w/ preprocessing', ...
title(['Dijkstra operations counts for ', dataset, 'X (X in [', ...
    num2str(sizesToPlot(1)), ', ', num2str(sizesToPlot(end)), '])'])
xlabel('Graph size')
