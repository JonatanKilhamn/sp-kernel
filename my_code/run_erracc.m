...
if sizeInd ~= 0
    ...
    for graphSize = sizesToRun
        ...
        if doSampling
            ...
            disp('Computing sampling kernels errors and acc.s')
        ...
        if doWL
            disp('Computing Weisfeiler-Lehman accuracy')
            
            wlK = wlKrnValues;
            [res] = runsvm(wlK, labels);
            wlKrnAccuracy = res.mean_acc;
            %optKernel = res.optkernel;
            
            disp('Finished W-L accuracy');
            if appending
                save(accFilename, 'wlKrnAccuracy', '-append');
            else
                save(accFilename, 'wlKrnAccuracy');
                appending = 1;
            end
        end
        ...
        if doGraphlets
            disp('Computing graphlet kernel accuracy')
            
            graphletAccuracy = zeros(nMValues, nTrials);
            %optKernel = res.optkernel;
            for i = 1:nMValues
                for j = 1:nTrials
                    % Accuracy
                    cellK{1} = graphletKrnValues{i, j};
                    [res] = runsvm(cellK, labels);
                    graphletAccuracy(i, j) = res.mean_acc;
                end
            end
            graphletAvgAccuracy = mean(graphletAccuracy, 2);
            disp('Finished graphlet accuracy');
            if appending
                save(accFilename, 'graphletAvgAccuracy', '-append');
            else
                save(accFilename, 'graphletAvgAccuracy');
                appending = 1;
            end
        end
