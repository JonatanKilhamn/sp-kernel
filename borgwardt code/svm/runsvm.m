...
for k = 1:cv
  ...
  for h=1:height
    ...
    for i = 1:size(cvalues,2)
      ...
      model = svmtrain(lk_current(1:p80,1), K1(1:p80,1:p80+1), strcat(['-q -t 4  -c ' num2str(cvalues(i))]));
      [predict_label, accuracy, dec_values] = ...
          svmpredict(lk_current(p80+1:p90,1), ...
          K1(p80+1:p90,1:p80+1), model, '-q');
  ...
  model = svmtrain(lk_current(1:p90,1), K1(1:p90,1:p90+1), ...
      strcat(['-q -t 4  -c ' num2str(cvalues(indoptimalc))]) );
  [predict_label, accuracy, dec_values] = ...
      svmpredict(lk_current(p90+1:size(K,1),1), ...
      K1(p90+1:size(K,1),1:p90+1), model, '-q');
  res.accuracy(k)=accuracy(1);
  
%   disp(['Finished c-v fold ', num2str(k), ' out of ', ...
%       num2str(cv)]);
