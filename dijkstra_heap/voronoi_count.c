 ...
 *
 *		[grouping, vorAdj, ops] = voronoi(adj, size, vorNodes)
 *
 *
 * Input:
...
void relax(int u, int v, double w, double key[], int handle[], 
           int heap_index[], int size, int pi[], double dist[],
           double grouping[], int *opsPtr) {
...
void initialize_multi_source(double key[], int handle[], int heap_index[],
			      int pi[], double dist[], double grouping[],
			      int vor[], int n, int nVor, int *opsPtr) {
  int i;
  for (i = 1; i <= n; ++i) {
    key[i] = 1000000000.0;
    dist[i] = 1000000000.0;
...
void vor_dijkstra(int first[], int node[], int next[], double w[], double d[],
	      int pi[], double dist[], double grouping[], int vor[], int n,
	      int nVor, int handle[], int heap_index[], int *opsPtr) {
  int size = n;
  int u, v, i;

  initialize_multi_source(d, handle, heap_index, pi, dist, grouping, vor, n,
                          nVor, opsPtr);
  while (size > 0) {
    u = handle[1];
...
/* Call dijkstra */
vor_dijkstra(first, node, next, w, d, pi, dist, grouping, vorNodes, size,
             nVorNodes, handle, heap_index, opsPtr);
